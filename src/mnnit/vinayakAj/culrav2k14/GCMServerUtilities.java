...
import static mnnit.vinayakAj.culrav2k14.Config.USE_LOCATION;
...
/**
 * A simple helper data class used to store data, register and unregister with
 * the AirBop servers.
 * 
 * This class contains all the necessary code to register and unregister with
 * the AirBop servers. It also shows how to format the headers and body elements
 * when posting to AirBop.
 * 
 * You are free to use it within your app, or translate the code as necessary to
 * fit your needs.
 */
public class GCMServerUtilities {

	/**
	 * Simple Exception class to store HTTP response codes returned by the
	 * AirBop serves in case of an error.
	 */
	...
	private static final int TIMOUT_MILLI_SECONDS = 1000 * 60;
	private static final String PREFERENCES = "mnnit.aj.vinayak.androidsample.data";
	...
	public GCMServerUtilities(final String regId) {
		mRegId = regId;
	}
	 ...
	 * 
	 * @param regId
	 *            Registration ID that we will store in the created instance.
	 * @return AirBopServerData
	 */
	public static GCMServerUtilities fillDefaults(final String regId) {
		GCMServerUtilities server_data = new GCMServerUtilities(regId);
		if (server_data != null) {
	...
	/**
	 * Fill a list with the current data values in alphabetical order. Used to
	 * construct the body when posting.
	 * 
	 * @param list_params
	 *            Will be filled with the data. A list of pairs.
	 * @param isRegister
	 *            Is this a registration or an unregistration? This controls
	...
	public void fillAlphaPairList(List<Pair<String, String>> list_params,
		...
		if ((isRegister) && (mLocation != null)) {
			Float latitude = Float.valueOf(Double.toString(mLocation
					.getLatitude()));
			Float longitude = Float.valueOf(Double.toString(mLocation
					.getLongitude()));
	...
	/**
	 * Load the necessary data from the prefs. We will load the label, latitude,
	 * and the longitude. The lat and long will be converted into a location
	 * object and used to set the country and state.
	 * 
	 * @param context
	 *            Context used to get the prefs
	 */
	...
	/**
	 * Save the current server data to the prefs so that the intent service can
	 * read them. We store the label, latitude, and the longitude.
	 * 
	 * @param context
	 *            Context needed to access the preferences
	 */
	 ...
	 * 
	 * @param context
	 *            The context needed to get the shared prefs
	 */
	...
	/**
	 * Calculate the current timestamp since the Epoch, January 1, 1970 00:00
	 * UTC.
	 * 
	 ...
	 * 
	 * @param method
	 *            The method being use to contact the header. POST for now.
	 * @param url
	 *            The URL we are going to contact.
	 * @param timestamp
	 *            The timestamp of the message
	 * @param body
	 *            The body that we will be sending
	 * @return The created URI.
	 */
	private String constructSignatureURI(final String method, final String url,
			final String timestamp, final String body) {
		/**
		 * The signature is constructed using the following value: "POST" +
		 * request_uri + APP_KEY + timestamp + request.body + APP_SECRET
		 */
		...
		// APP_KEY
		uriBuilder.append(APP_KEY);
		// timestamp
		uriBuilder.append(timestamp);
		// body
		uriBuilder.append(body);
		// ssssshhhhhh
		uriBuilder.append(APP_SECRET);
	 ...
	 * 
	 * @param isRegister
	 *            Is this a registration or an unregistration? Controls the
	 *            parameters that will be in the body
	 ...
	 * 
	 * @param isRegister
	 *            A registration or uneregistration?
	 * @param asJSON
	 *            Do we want a JSON body or a url encoded body?
	 * @return The body
	 */
	 ...
	 *            The URL to post to
	 * @param isRegister
	 *            IS this a registration or unregistration?
	 * @param asJSON
	 *            Do we want the post body as JSON or URL encoded
	 * @throws IOException
	 * @throws AirBopException
	 *             Used to return the HTTP reponse code.
	 */
	public void post(String url_endpoint, final boolean isRegister,
			final boolean asJSON) throws IOException, AirBopException {
		URL url;
		try {
		...
		String signature = constructSignatureURI("POST", url_endpoint,
				timestamp, body);
		...
		try {
			...
			conn.setConnectTimeout(TIMOUT_MILLI_SECONDS);
			conn.setReadTimeout(TIMOUT_MILLI_SECONDS);
			...
			conn.addRequestProperty(HEADER_APP, APP_KEY);
	 ...
	 * 
	 * @param context
	 *            Just used for messages
	 * @param server_data
	 *            Server data containing info to register with
	 * @return success or failure
	 */
	static boolean register(final Context context,
			final GCMServerUtilities server_data) {
		if (context == null) {
			return false;
		}
		...
		for (int i = 1; i <= MAX_ATTEMPTS; i++) {
			...
			try {
				 ...
				 */
				/*
				 * long lifespan = GCMRegistrar
				 * .getRegisterOnServerLifespan(context); //long expirationTime
				 * = System.currentTimeMillis() + lifespan;
				 * 
				 * /*displayMessage(context, "Registration expires on: " + new
				...
				if ((error_code >= 400) && (error_code <= 499)) {
					/*
					 * displayMessage(context, context.getString(
					 * R.string.request_error, error_code, error_msg));
					 */
					return false;
				} else {
					...
					/*
					 * displayMessage(context, context.getString(
					 * R.string.server_reg_failed, error_code, error_msg));
					 */

					// Here we are retrying on any error code that is not a 4XX
					// error code. This will most probably be a 5XX error code
					// meaning
				...
				/*
				 * displayMessage( context,
				 * context.getString(R.string.server_reg_failed_timeout,
				 * e.getMessage()));
				 */
				...
				/*
				 * displayMessage( context,
				 * context.getString(R.string.server_reg_failed_401,
				 * e.getMessage()));
				 */
				return false;
			}
	...
	static boolean unregister(final Context context, final String regId) {
		...
		// displayMessage(context,
		// context.getString(R.string.unregister_device));
		...
		try {
			...
			/*
			 * Log.e(TAG, "Failed to unregister on attempt " + e); String
			 * message = context.getString( R.string.server_unregister_error,
			 * e.getMessage()); //displayMessage(context, message);
			 */
			return false;
		}
		return true;
